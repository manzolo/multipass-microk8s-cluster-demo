...
generate_nginx_config() {
    ...
    node_instances=$(multipass list | grep "$VM_NODE_PREFIX" | awk '{print $1}' || true)
...
add_nginx_dns_entries() {
    local VM_IP
    VM_IP=$(get_vm_ip "$LOAD_BALANCE_HOSTNAME")
...
function destroy_nginx_lb() {
    ...
    restart_dns_service
    multipass stop --force "$LOAD_BALANCE_HOSTNAME" > /dev/null 2>&1 || true
    multipass delete --purge "$LOAD_BALANCE_HOSTNAME" > /dev/null 2>&1 || true
